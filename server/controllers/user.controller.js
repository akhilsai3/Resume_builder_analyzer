...
const updateUser = async (req, res) => {
 ...
 // Nuclear validation - reject if somehow already hashed
 if (password && (password.startsWith('$2a$') || 
 password.startsWith('$2b$'))) {
return res.status(400).json({
message: "Server error: Received pre-hashed password"
});
}
  ...
  try {
    console.log('Starting update process...'); // Debug log
      ...
      if (password) {
          ...
          console.log('Setting new password...'); // Debug log
          // This assignment will trigger the setter function
      ...
      // This .save() will ensure the setter function runs
      const updatedUser = await user.save();
console.log('User saved, password hash:', updatedUser.password); 
      res.json({
          success: true,
